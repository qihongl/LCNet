...
class PEKDECRP():
    ...
    use KDE to compute likelihood and combine with CRP prior to get posterior
    '''
    ...
    def assign_to_cluster(self, x_t):
        # compute the log lik for exisiting clusters
        kdes = [None] * len(self.context)
        log_liks_mu = np.zeros(len(self.context))
        log_liks_se = np.zeros(len(self.context))
        log_lik_new = np.zeros(len(self.context))
        for k in range(len(self.context)):
            # for cluster k
            mask_k = np.array(np.array(self.cluster_assignment) == k)
            # establish its KDE
            kdes[k] = KernelDensity(kernel='gaussian', bandwidth=0.2).fit(np.array(self.x_history)[mask_k,:])
            # establish its in-cluster likelihood baseline
            log_lik_k = kdes[k].score_samples(np.array(self.x_history)[mask_k,:])
            log_liks_mu[k], log_liks_se[k] = compute_stats(log_lik_k)
            # compute the log lik for the new point
            log_lik_new[k] = kdes[k].score_samples(x_t.reshape(1,-1))
        ...
        # if it is unlikely to come from an exisiting cluster...
        if np.all(log_lik_new < (log_liks_mu - 5 * log_liks_se)):
            # create a new cluster
            self.add_new_context()
            self.cluster_assignment.append(len(self.context)-1)
        else:
            # assign to the max lik cluster
            print(log_lik_new / log_liks_mu)
            max_lik_cluster_id = np.argmax(log_lik_new / log_liks_mu)
            self.cluster_assignment.append(max_lik_cluster_id)
        ...
        # print(self.counts)
        # print(self.cluster_assignment[-1])
...
if __name__ == "__main__":
    ...
    for subj_id in range(10):
        ...
        dist_threshold = .1
        pe_threshold = 3
        pkcrp = PEKDECRP(dist_threshold, pe_threshold)
        ...
        total_score += np.mean(ctx_ids == pkcrp.cluster_assignment)
    print(total_score)
